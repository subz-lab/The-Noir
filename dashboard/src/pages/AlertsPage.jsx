...
import { Activity, ShieldAlert, CheckCircle, ChevronRight, RefreshCcw, Filter } from 'lucide-react';
...
const AlertsPage = ({ logs, isLoading, onRefresh }) => {
    const [selectedAlert, setSelectedAlert] = useState(null);
    const [filterLevel, setFilterLevel] = useState('ALL'); // ALL, THREAT, SUSPICIOUS

    // Filter logs to only show Suspicious or Threat levels
    const alerts = logs.filter(log => {
        const lbl = (log.label || '').toUpperCase();
        if (['NORMAL', 'SAFE'].includes(lbl)) return false; // Never show normal logs in alerts
        ...
        <div className="w-full h-full animate-in fade-in zoom-in-95 duration-500 flex flex-col">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-3xl font-bold text-white font-grotesk tracking-tight">Active Alerts</h2>
                        <p className="text-sm text-white/40">Critical vectors and anomalous behaviors requiring attention</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 bg-white/5 p-1 rounded-2xl border border-white/10">
                    {['ALL', 'THREAT', 'SUSPICIOUS'].map(level => (
                        <button
                            ...
                            className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${filterLevel === level ? 'bg-white text-black shadow-lg' : 'text-white/40 hover:text-white/80'}`}
                        >
                            {level}
                        </button>
                    ))}
                    <div className="w-px h-6 bg-white/10 mx-2" />
                    <button onClick={onRefresh} className="p-2 mr-1 rounded-xl hover:bg-white/10 text-white/60 transition-colors">
                        <RefreshCcw className="w-4 h-4" />
                    </button>
                </div>
            </div>

            <div className="grid grid-cols-12 gap-8 flex-1 pb-12">

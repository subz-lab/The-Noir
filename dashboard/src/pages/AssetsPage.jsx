...
import { Database, Search, ShieldCheck, ShieldAlert, Cpu } from 'lucide-react';
import SentinelGrid, { SentinelSection } from '../components/SentinelGrid';
import Skeleton from '../components/Skeleton';

const AssetsPage = ({ logs, isLoading, onRefresh }) => {
        ...
        logs.forEach(log => {
            ...
            if (!ipMap.has(log.ip_address)) {
                ipMap.set(log.ip_address, {
                    ...
                    // Simulate system type based on ports
        ...
        <div className="w-full h-full animate-in fade-in zoom-in-95 duration-500 flex flex-col">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-3xl font-bold text-white font-grotesk tracking-tight">Protected Assets</h2>
                        <p className="text-sm text-white/40">Monitored Nodes & Infrastructure Health</p>
                    </div>
                </div>
            ...
            <SentinelGrid>
                <SentinelSection id="asset-inventory" colSpan="col-span-12" title="Topology Inventory">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {isLoading ? (
                            [...Array(6)].map((_, i) => <Skeleton key={i} className="h-40 w-full rounded-3xl" />)
                        ) : filteredAssets.length === 0 ? (
                            <div className="col-span-12 text-center py-24 text-white/30 uppercase tracking-widest font-bold">
